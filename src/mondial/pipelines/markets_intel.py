...
`snapshot.json` for the frontend.
...
def main() -> int:
    ...
    predictions = predict_upcoming() or []                    # persist + snapshot input
    write_snapshot(predictions)
    place_virtual_bets()                                       # six-player bets @ Winner line
    publish_web()                                              # refresh static-site feed
    log.info("markets_intel complete: %d predictions, snapshot + bets written",
             len(predictions))
    return 0
